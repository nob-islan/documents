...
auth_handler.go と同じパッケージに auth_handler_test.go を作成します:
...
func (m *mockAuthUsecase) Me(in payload.MeIn) payload.MeOut {
	args := m.Called(in)
	return args.Get(0).(payload.MeOut)
}
...
func Test_AuthHandler_Me(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(mock *mockAuthUsecase) {
				mock.On("Me", payload.NewMeIn("nob")).Return(payload.NewMeOut("nob", 13))
			},
			expectedStatusCode: http.StatusOK,
...
auth_usecase.go と同じパッケージに auth_usecase_test.go を作成します:
...
func (m *mockUsersRepository) FindByName(name string) (domain.Users, error) {
	args := m.Called(name)
...
func Test_AuthUsecase_Me(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			// usecaseの実行
			result := NewAuthUsecase(mockRepository).Me(testcase.requestBody)
...
func Test_UsersRepository_FindByName(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			// repositoryの実行
			result, err := NewUsersRepository(db).FindByName(testcase.queryParam)
