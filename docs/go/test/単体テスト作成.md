...
func (m *mockUserUsecase) GetUser(ctx context.Context, in params.GetUserIn) (params.GetUserOut, error) {
	args := m.Called(ctx, in)
	return args.Get(0).(params.GetUserOut), args.Error(1)
}
	...
	}{
		{
			...
			setupMock: func(m *mockUserUsecase) {
				m.On(
					"GetUser",
					mock.Anything,
					params.NewGetUserIn("nob"),
				).Return(
					params.NewGetUserOut("nob", 13),
					nil,
				)
			},
			expectedStatusCode: http.StatusOK,
		...
		},
		{
			...
			setupMock: func(m *mockUserUsecase) {
				m.On(
					"GetUser",
					mock.Anything,
					params.NewGetUserIn("nobody"),
				).Return(
					*new(params.GetUserOut),
...
func TestGetUser(t *testing.T) {
	...
	tests := []struct {
		name          string                      // テストケース名
		requestBody   params.GetUserIn            // リクエストボディ
		setupMock     func(m *mockUserRepository) // モック設定
		expectedBody  params.GetUserOut           // 期待されるレスポンスボディ
		expectedError error                       // 期待されるエラー
	}{
		{
			name:        "Success",
			requestBody: params.NewGetUserIn("nob"),
			setupMock: func(m *mockUserRepository) {
				m.On(
					"FindByName",
					mock.Anything,
			...
			expectedBody:  params.NewGetUserOut("nob", 13),
			expectedError: nil,
		},
		{
			name:        "NoSuchUser",
			...
			expectedBody:  params.GetUserOut{},
