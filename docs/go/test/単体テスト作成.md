...
func (m *mockUsersRepository) FindByName(q domain.FindByNameQuery) domain.Users {
	args := m.Called(q)
...
func Test_UsersRepository_FindByName(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			// repositoryの実行
			result := NewUsersRepository(mockSql).FindByName(testcase.query)
