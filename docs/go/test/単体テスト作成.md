...
func (m *mockUserRepository) FindByName(q domain.FindByNameQuery) domain.User {
	args := m.Called(q)
	return args.Get(0).(domain.User)
}

// UserUsecase_Loginのテスト
func Test_UserUsecase_Login(t *testing.T) {

	tests := []struct {
		name         string                          // テストケース名
		requestBody  params.LoginIn                  // リクエストボディ
		setupMock    func(mock *mockUserRepository) // モック設定
	...
	}{
		{
			...
			setupMock: func(mock *mockUserRepository) {
				mock.On("FindByName", domain.NewFindByNameQuery("nob")).Return(
	...
	for _, testcase := range tests {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			// usecaseの実行
			result := NewUserUsecase(mockRepository).Login(testcase.requestBody)
...
// UserUsecase_Meのテスト
func Test_UserUsecase_Me(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			// usecaseの実行
			result, err := NewUserUsecase(mockRepository).Me(testcase.requestBody)
...
`users_repository.go`と同じパッケージにテストファイルを作成します。
...
func Test_UserRepository_FindByName(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			// repositoryの実行
			result := NewUserRepository(mockSql).FindByName(testcase.query)
...
// UsersSql_FindByNameのテスト
func Test_UsersSql_FindByName(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		t.Run(testcase.name, func(t *testing.T) {
			...
			// sqlの実行
			result := NewUsersSql(db).FindByName(testcase.queryParam)
