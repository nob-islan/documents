...
func ToHttpErrorResponse(err error) (int, any) {
	...
	case apperrors.BusinessError:
		return http.StatusUnprocessableEntity, businessErrorResponse{Message: e.Error()}
	default:
...
type businessErrorResponse struct {
	Message string `json:"message" example:"invalid param"` // エラーメッセージ
}
