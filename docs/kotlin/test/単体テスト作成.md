...
class AuthControllerTest {
    ...
    fun testGetUserSuccess() {
        ...
        // リクエスト作成
        val req = GetUserRequest("nob")

        // serviceのモック化
        whenever(authService.getUser(GetUserInModel(req.name))).thenReturn(GetUserOutModel("nob", 13))
        ...
        val result = mockMvc.perform(
            ...
            .andReturn()
        // 結果の検証
        assertThat(result.response.contentAsString).isEqualTo(objectMapper.writeValueAsString(GetUserResponse("nob", 13)))
    }
}
```
...
class AuthServiceImplTest {
    ...
    fun testGetUserSuccess() {
        ...
        // 入力値の作成
        val inModel = GetUserInModel("nob")

        // repositoryのモック化
        whenever(usersRepository.findByName(inModel.name)).thenReturn(Users("nob", "passwd", 13))

        // service呼び出し
        val outModel = authService.getUser(inModel)
        // 結果の検証
