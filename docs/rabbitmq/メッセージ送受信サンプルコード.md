...
RabbitMQ のメッセージの送信・受信を行う golang のサンプルコードです。
...
実行すると、下記のようなメッセージが表示され、RabbitMQ の Web 画面上でキューが作成されたことが確認できます:
...
`select{}` によって、起動後アプリは待機を続けます。RabbitMQ からメッセージが push されると下記のようなメッセージが出力されます:
